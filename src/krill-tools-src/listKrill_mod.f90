!! listKrill_mod.f90

module listKrill_mod
	use class_krill
	use abstList_mod
	use netcdf

	private
	public :: listKrill
	type, extends(list) :: listKrill 
		contains
			! functions
			procedure :: n_krill ! count number of krills in list
			procedure, public :: current => currentKrill ! get krill pointed by iterator

			! subroutines
			procedure, public :: addKrill ! add krill in list
			procedure, public :: printList => printListKrill ! print the attributes of all krills in list
			procedure, public :: thisKrill ! allow acces to a specific krill 
			procedure, public :: netcdfKrill ! generate a netcdf file for list of krills "much fun! such Krills!"
	end type listKrill

contains
	integer function n_krill(this)
		class(listKrill) :: this
		integer :: n

		n = 0
		
		call this%reset()
		do while(this%moreValues())
			n = n + 1
			call this%next()
		end do

		n_krill = n
	end function n_krill

	subroutine thisKrill(this, krillPosition)
		class(listKrill) :: this
		integer :: krillPosition

		call this%reset()
		do i =  1, (krillPosition - 1)
			call this%next()
		end do
	end subroutine thisKrill
	
	subroutine printListKrill(this)
		class(listKrill) :: this
		class(*), pointer :: curr

		call this%reset()

		do while(this%moreValues())
			curr => this%currentValue()

			select type(curr)
			type is (Krill)
				call curr%debug()
			end select
			
			call this%next()
		end do
		
		call this%reset()
	end subroutine printListKrill

	subroutine addKrill(this, value)
		class(listKrill) :: this
		class(Krill) :: value
		class(*), allocatable :: v

		allocate(v, source = value)
		call this%addValue(v)
	end subroutine addKrill

	function currentKrill(this)
		type(Krill) :: currentKrill
		class(listKrill) :: this
		class(*), pointer :: v

		v => this%currentValue()
		select type(v)
		type is (Krill)
			currentKrill = v
		end select
	end function currentKrill

	subroutine netcdfKrill(this, datasetNameRead, datasetNameWrite, time, jumpTime)
		class(listKrill) :: this
		class(*), pointer :: curr

		! iterator
		integer :: i, j, k

		! variables for the netcdf procedures
		character(*), intent(in) :: datasetNameWrite, datasetNameRead
		integer :: n
		character(30) :: path_to_dataset
		integer :: ioerr, time, jumpTime
		integer(kind = 4) :: file_ID, numberKrillID, attributesKrillID, specieID, sexID, sizeID, massID, &
				&	dev_freqID, molt_sizeID, awID, bwID, eiID, a_moltID, b_moltID, k0ID, h0ID, &
				&	AID, r0ID, p_zooID, p_phytoID, w_moltID, matrixID, timeID
		integer, dimension(2) :: matrixDim, numberTime

		! buffer arrays to allocate values to netcdf variables
		integer, dimension(:,:), allocatable :: all_specie
		integer, dimension(:,:), allocatable :: all_sex
		real, dimension(:,:), allocatable :: all_size
		real, dimension(:,:), allocatable :: all_mass
		real, dimension(:,:), allocatable :: all_dev_freq
		real, dimension(:,:), allocatable :: all_molt_size
		real, dimension(:,:), allocatable :: all_aw
		real, dimension(:,:), allocatable :: all_bw
		real, dimension(:,:), allocatable :: all_ei
		real, dimension(:,:), allocatable :: all_a_molt
		real, dimension(:,:), allocatable :: all_b_molt
		real, dimension(:,:), allocatable :: all_k0
		real, dimension(:,:), allocatable :: all_h0
		real, dimension(:,:), allocatable :: all_A
		real, dimension(:,:), allocatable :: all_r0
		real, dimension(:,:), allocatable :: all_p_zoo
		real, dimension(:,:), allocatable :: all_p_phyto
		real, dimension(:,:), allocatable :: all_w_molt

		path_to_dataset = "../KrillModelNETCDF/"
		n = this%n_krill()
		numberTime = time/jumpTime
		print *, "number of krills = ", this%n_krill()

		print *, "allocating memory for buffer arrays"
		allocate(all_specie(n, numberTime))
		allocate(all_sex(n, numberTime))
		allocate(all_size(n, numberTime))
		allocate(all_mass(n, numberTime))
		allocate(all_dev_freq(n, numberTime))
		allocate(all_molt_size(n, numberTime))
		allocate(all_aw(n, numberTime))
		allocate(all_bw(n, numberTime))
		allocate(all_ei(n, numberTime))
		allocate(all_a_molt(n, numberTime))
		allocate(all_b_molt(n, numberTime))
		allocate(all_k0(n, numberTime))
		allocate(all_h0(n, numberTime))
		allocate(all_A(n, numberTime))
		allocate(all_r0(n, numberTime))
		allocate(all_p_zoo(n, numberTime))
		allocate(all_p_phyto(n, numberTime))
		allocate(all_w_molt(n, numberTime))

		! Creation of the netcdf dataset
		print *, "creation of netcdf file : ", datasetNameWrite
		ioerr = nf90_create(path = trim(path_to_dataset)//datasetNameWrite, cmode = nf90_clobber, ncid = file_ID)
		if (ioerr /= nf90_noerr) print*,'173',nf90_strerror(ioerr)

		! define the dimension
		ioerr = nf90_def_dim(file_ID, "numberKrill", n, numberKrillID)
		if (ioerr /= nf90_noerr) print*, 'NUMBERKRILL_ERROR', nf90_strerror(ioerr)
		ioerr = nf90_def_dim(file_ID, "time", nf90_unlimited, timeID)
		if (ioerr /= nf90_noerr) print*, 'TIME_ERROR', nf90_strerror(ioerr)	

		! creation of the variables for the files
		print *, "creation of file variables"
		ioerr = nf90_def_var(file_ID, "specie", nf90_int, (/numberKrillID, timeID/), specieID)
		if (ioerr /= nf90_noerr) print *, 'CRE_SPECIE_ERROR',nf90_strerror(ioerr)
		ioerr = nf90_def_var(file_ID, "sex", nf90_int, (/numberKrillID, timeID/), sexID)
		if (ioerr /= nf90_noerr) print *, 'CRE_SEX_ERROR',nf90_strerror(ioerr)
		ioerr = nf90_def_var(file_ID, "size", nf90_float, (/numberKrillID, timeID/), sizeID)
		if (ioerr /= nf90_noerr) print *, 'CRE_SIZE_ERROR',nf90_strerror(ioerr)
		ioerr = nf90_def_var(file_ID, "mass", nf90_float, (/numberKrillID, timeID/), massID)
		if (ioerr /= nf90_noerr) print *, 'CRE_MASS_ERROR',nf90_strerror(ioerr)
		ioerr = nf90_def_var(file_ID, "dev_freq", nf90_float, (/numberKrillID, timeID/), dev_freqID)
		if (ioerr /= nf90_noerr) print *, 'CRE_DEV_FREQ_ERROR',nf90_strerror(ioerr)
		ioerr = nf90_def_var(file_ID, "molt_size", nf90_float, (/numberKrillID, timeID/), molt_sizeID)
		if (ioerr /= nf90_noerr) print *, 'CRE_MOLT_SIZE_ERROR',nf90_strerror(ioerr)
		ioerr = nf90_def_var(file_ID, "aw", nf90_float, (/numberKrillID, timeID/), awID)
		if (ioerr /= nf90_noerr) print *, 'CRE_AW_ERROR',nf90_strerror(ioerr)
		ioerr = nf90_def_var(file_ID, "bw", nf90_float, (/numberKrillID, timeID/), bwID)
		if (ioerr /= nf90_noerr) print *, 'CRE_BW_ERROR',nf90_strerror(ioerr)
		ioerr = nf90_def_var(file_ID, "ei", nf90_float, (/numberKrillID, timeID/), eiID)
		if (ioerr /= nf90_noerr) print *, 'CRE_EI_ERROR',nf90_strerror(ioerr)
		ioerr = nf90_def_var(file_ID, "a_molt", nf90_float, (/numberKrillID, timeID/), a_moltID)
		if (ioerr /= nf90_noerr) print *, 'CRE_A_MOLT_ERROR',nf90_strerror(ioerr)
		ioerr = nf90_def_var(file_ID, "b_molt", nf90_float, (/numberKrillID, timeID/), b_moltID)
		if (ioerr /= nf90_noerr) print *, 'CRE_B_MOLT_ERROR',nf90_strerror(ioerr)
		ioerr = nf90_def_var(file_ID, "k0", nf90_float, (/numberKrillID, timeID/), k0ID)
		if (ioerr /= nf90_noerr) print *, 'CRE_K0_ERROR',nf90_strerror(ioerr)
		ioerr = nf90_def_var(file_ID, "h0", nf90_float, (/numberKrillID, timeID/), h0ID)
		if (ioerr /= nf90_noerr) print *, 'CRE_H0_ERROR',nf90_strerror(ioerr)
		ioerr = nf90_def_var(file_ID, "A", nf90_float, (/numberKrillID, timeID/), AID)
		if (ioerr /= nf90_noerr) print *, 'CRE_A_ERROR',nf90_strerror(ioerr)
		ioerr = nf90_def_var(file_ID, "r0", nf90_float, (/numberKrillID, timeID/), r0ID)
		if (ioerr /= nf90_noerr) print *, 'CRE_R0_ERROR',nf90_strerror(ioerr)
		ioerr = nf90_def_var(file_ID, "p_zoo", nf90_float, (/numberKrillID, timeID/), p_zooID)
		if (ioerr /= nf90_noerr) print *, 'CRE_P_ZOO_ERROR',nf90_strerror(ioerr)
		ioerr = nf90_def_var(file_ID, "p_phyto", nf90_float, (/numberKrillID, timeID/), p_phytoID)
		if (ioerr /= nf90_noerr) print *, 'CRE_P_PHYTO_ERROR',nf90_strerror(ioerr)
		ioerr = nf90_def_var(file_ID, "w_molt", nf90_float, (/numberKrillID, timeID/), w_moltID)
		if (ioerr /= nf90_noerr) print *, 'CRE_W_MOLT_ERROR',nf90_strerror(ioerr)

		! end definition of file
		ioerr = nf90_enddef(file_ID)	
		if (ioerr /= nf90_noerr) print*,'DEF_ERROR',nf90_strerror(ioerr)

		! filling up dimesions and values
		j = 1

		do k = 1, time
			call this%reset()
		
			i = 1

			print *, "putting values in buffer arrays"

			if (mod(real(k), real(jumpTime)) == 0)
				do while(this%moreValues())
					curr => this%currentValue()
			
					select type(curr)
					type is (Krill)
						all_specie(i, j) = curr%get_specie()
						all_sex(i, j) = curr%get_sex()
						all_size(i, j) = curr%get_size()
						all_mass(i, j) = curr%get_mass()
						all_dev_freq(i, j) = curr%get_dev_freq()
						all_molt_size(i, j) = curr%get_molt_size()
						all_aw(i, j) = curr%get_aw()
						all_bw(i, j) = curr%get_bw()
						all_ei(i, j) = curr%get_ei()
						all_a_molt(i, j) = curr%get_a_molt()
						all_b_molt(i, j) = curr%get_b_molt()
						all_k0(i, j) = curr%get_k0()
						all_h0(i, j) = curr%get_h0()
						all_A(i, j) = curr%get_A()
						all_r0(i, j) = curr%get_r0()
						all_p_zoo(i, j) = curr%get_p_zoo()
						all_p_phyto(i, j) = curr%get_p_phyto()
						all_w_molt(i, j) = curr%get_w_molt()
					end select
			
					i = i + 1
					call this%next()
				end do

				j = j + 1	

				! allocate values to variables
				print *, "allocating values to file variables"
				ioerr = nf90_put_var(file_ID, specieID, all_specie, (/1, 1/), (/n, numberTime/))
				if (ioerr /= nf90_noerr) print *, 'PUT_SPECIE_ERROR',nf90_strerror(ioerr)
				ioerr = nf90_put_var(file_ID, sexID, all_sex, (/1, 1/), (/n, numberTime/))
				if (ioerr /= nf90_noerr) print *, 'PUT_SEX_ERROR',nf90_strerror(ioerr)
				ioerr = nf90_put_var(file_ID, sizeID, all_size, (/1, 1/), (/n, numberTime/))
				if (ioerr /= nf90_noerr) print *, 'PUT_SIZE_ERROR',nf90_strerror(ioerr)
				ioerr = nf90_put_var(file_ID, massID, all_mass, (/1, 1/), (/n, numberTime/))
				if (ioerr /= nf90_noerr) print *, 'PUT_MASS_ERROR',nf90_strerror(ioerr)
				ioerr = nf90_put_var(file_ID, dev_freqID, all_dev_freq, (/1, 1/), (/n, numberTime/))
				if (ioerr /= nf90_noerr) print *, 'PUT_DEV_FREQ_ERROR',nf90_strerror(ioerr)
				ioerr = nf90_put_var(file_ID, molt_sizeID, all_molt_size, (/1, 1/), (/n, numberTime/))
				if (ioerr /= nf90_noerr) print *, 'PUT_MOLT_SIZE_ERROR',nf90_strerror(ioerr)
				ioerr = nf90_put_var(file_ID, awID, all_aw, (/1, 1/), (/n, numberTime/))
				if (ioerr /= nf90_noerr) print *, 'PUT_AW_ERROR',nf90_strerror(ioerr)
				ioerr = nf90_put_var(file_ID, bwID, all_bw, (/1, 1/), (/n, numberTime/))
				if (ioerr /= nf90_noerr) print *, 'PUT_BW_ERROR',nf90_strerror(ioerr)
				ioerr = nf90_put_var(file_ID, eiID, all_ei, (/1, 1/), (/n, numberTime/))
				if (ioerr /= nf90_noerr) print *, 'PUT_EI_ERROR',nf90_strerror(ioerr)
				ioerr = nf90_put_var(file_ID, a_moltID, all_a_molt, (/1, 1/), (/n, numberTime/))
				if (ioerr /= nf90_noerr) print *, 'PUT_A_MOLT_ERROR',nf90_strerror(ioerr)
				ioerr = nf90_put_var(file_ID, b_moltID, all_b_molt, (/1, 1/), (/n, numberTime/))
				if (ioerr /= nf90_noerr) print *, 'PUT_B_MOLT_ERROR',nf90_strerror(ioerr)
				ioerr = nf90_put_var(file_ID, k0ID, all_k0, (/1, 1/), (/n, numberTime/))
				if (ioerr /= nf90_noerr) print *, 'PUT_K0_ERROR',nf90_strerror(ioerr)
				ioerr = nf90_put_var(file_ID, h0ID, all_h0, (/1, 1/), (/n, numberTime/))
				if (ioerr /= nf90_noerr) print *, 'PUT_H0_ERROR',nf90_strerror(ioerr)
				ioerr = nf90_put_var(file_ID, AID, all_A, (/1, 1/), (/n, numberTime/))
				if (ioerr /= nf90_noerr) print *, 'PUT_A_ERROR',nf90_strerror(ioerr)
				ioerr = nf90_put_var(file_ID, r0ID, all_r0, (/1, 1/), (/n, numberTime/))
				if (ioerr /= nf90_noerr) print *, 'PUT_R0_ERROR',nf90_strerror(ioerr)
				ioerr = nf90_put_var(file_ID, p_zooID, all_p_zoo, (/1, 1/), (/n, numberTime/))
				if (ioerr /= nf90_noerr) print *, 'PUT_P_ZOO_ERROR',nf90_strerror(ioerr)
				ioerr = nf90_put_var(file_ID, p_phytoID, all_p_phyto, (/1, 1/), (/n, numberTime/))
				if (ioerr /= nf90_noerr) print *, 'PUT_P_PHYTO_ERROR',nf90_strerror(ioerr)
				ioerr = nf90_put_var(file_ID, w_moltID, all_w_molt, (/1, 1/), (/n, numberTime/))
				if (ioerr /= nf90_noerr) print *, 'PUT_W_MOLT_ERROR',nf90_strerror(ioerr)
			endif
			
			! making krills continue  their lives
			call evolvelist(this, datasetNameRead)
		end do
		! closing file
		ioerr = nf90_close(file_ID)
		if (ioerr /= nf90_noerr) print *, 'CLOSE_ERROR', nf90_strerror(ioerr) 

		! deallocating all allocatable arrays
		print *, "deallocating memory of buffer arrays"
		deallocate(all_specie)
		deallocate(all_sex)
		deallocate(all_size)
		deallocate(all_mass)
		deallocate(all_dev_freq)
		deallocate(all_molt_size)
		deallocate(all_aw)
		deallocate(all_bw)
		deallocate(all_ei)
		deallocate(all_a_molt)
		deallocate(all_b_molt)
		deallocate(all_k0)
		deallocate(all_h0)
		deallocate(all_A)
		deallocate(all_r0)
		deallocate(all_p_zoo)
		deallocate(all_p_phyto)
		deallocate(all_w_molt)

		print *, "creation of file (", datasetNameWrite, ") complete"
	end subroutine netcdfKrill
end module listKrill_mod
